          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4 text-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="p-3 rounded-md border border-border bg-muted/20 space-y-1">
                <span className="text-xs text-muted-foreground block">Đơn vị & Vị trí:</span>
              ...
              <div className="p-3 rounded-md border border-border bg-muted/20 space-y-1 sm:col-span-2">
                <span className="text-xs text-muted-foreground block">Lý do đề xuất:</span>
                ...
                <div className="space-y-1">
                  {selectedRequest.attachmentUrls.map((url, i) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-between p-2 rounded-md bg-muted/40 border border-border text-xs text-foreground hover:bg-muted transition-colors"
                    >
                      <span className="flex items-center gap-2 truncate max-w-[85%] font-mono text-xs">
                        <FileText className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                        {url}
                      </span>
                      <span className="text-xs text-muted-foreground underline">Mở liên kết ↗</span>
                    </a>
                  ))}
                </div>
              ) : (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {changedLabels.slice(0, 3).map((lbl, idx) => (
                            <span
                              key={idx}
                              className="px-1.5 py-0.5 rounded-md text-xs bg-muted text-foreground border border-border"
                            >
                              {lbl}
                            </span>
                          ))}
